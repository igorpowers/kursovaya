...
def shop(request):
    return render(request, 'shop/base.html', {"contents": Product.objects.all(), "categories":Category.objects.all()})
...
def category_slug(request, category_slug):
    for categorys in Category.objects.all():
        if str(category_slug) == str(categorys.slug):
            return render(request, 'shop/base.html', {"contents": Product.objects.filter(category_id=categorys.id), "categories":Category.objects.all()})
